...
void setUpFields(tab tabs[], int tabCounter, char* things[]){
    ...
    if(!tabCounter){
        ...
        tabs[tabCounter].fields[3].bar = create_progress_bar(tabs[tabCounter].tab, 0, -230, RPM_MAX, RPM_MIN);
...
void updateObject(field object, int value, float conversionFactor, int index, int page){
    ...
    if(page && (index == TAB2_RPM_INDEX)){
    ...
    else if(index == TAB1_GEAR_INDEX && newValue == 0){
    ...
    else if(index == TAB1_GEAR_INDEX && newValue > 6){
        if(strcmp(lv_label_get_text(object.counter), "D")){
            lv_label_set_text(object.counter, "D");
            countChanged = 1;
        }
        ...
        sprintf(buffer, " #ffffff %d ", (int)newValue);
        if(strcmp(lv_label_get_text(object.counter), buffer)){
            lv_label_set_text(object.counter, buffer);
            lv_bar_set_value(object.bar, newValue, LV_ANIM_OFF);
    ...
    for(int i = 0; i < messageCount; i++){
        ...
        switch(message.identifier){
            case 0x700:
                ...
                if((message.data[4])*VOLTAGE_CONVERSION<10){
                    VoltageError = 1;
                }
                else{
                    VoltageError = 0;
                }
                updateObject(tabs[TAB1].fields[TAB1_VOLTAGE_INDEX], (message.data[4]), VOLTAGE_CONVERSION, TAB1_VOLTAGE_INDEX, TAB1);
            ...
            case 0x702:
                if((int)((message.data[4] << 8 | message.data[5])*FRONT_BRAKE_PRESSURE_CONVERSION)>9000){
                    updateObject(tabs[TAB2].fields[TAB2_FRONT_BRAKE_PRESSURE_INDEX], 0, FRONT_BRAKE_PRESSURE_CONVERSION, TAB2_FRONT_BRAKE_PRESSURE_INDEX, TAB2);
                }
                else{
...
void warning(tab tabs[]){
    ...
    while(true){
        if(previousError != errorCode){
            ...
            if(errorCode && esp_timer_get_time()/1000 > 10000){//wait 10 seconds after startup to show errors
                if(lv_tabview_get_tab_act(tabview)){//switch to tab 1 if not already on it
                    lvgl_port_lock(-1);
                    switchTabView();
                ...
                for(int i = 0; i<3 && (errorCode); i++){//flash the screen 3 times, stop if error clears
                    lvgl_port_lock(-1);
                ...
                lv_obj_invalidate(tabs[lv_tabview_get_tab_act(tabview)].tab);
                lvgl_port_unlock();
                vTaskDelay(100);
            }
        }
