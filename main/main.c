...
void IRAM_ATTR gpio_isr_handler(void* arg) {
    gpio_intr_disable(GPIO_INPUT_IO);//debounce
    currentTab = !currentTab;//switch tab
    // disabled = 1;
}
...
void app_main(){
    ...
    if (lvgl_port_lock(-1)) {
        ...
        //**********************************************************************************
        //Screen setup and other initializations go here
        lvgl_port_lock(-1);
        ...
        gpio_isr_handler_add(GPIO_INPUT_IO, gpio_isr_handler, (void*) &currentTab);

        twai_message_t messageArray[6] = {0};
        int messageNumber = 0;
        twai_message_t tempMessage;
        int messageIdentifiers[] = {0, 1, 2, 4, 15, 20};
        int messageCount = sizeof(messageIdentifiers)/sizeof(messageIdentifiers[0]);
        lvgl_port_unlock();
        //**********************************************************************************
        ...
        while(true){
            ...
            if(currentTab != previousTab){
                ...
                if(!gpio_get_level(GPIO_INPUT_IO)){
                    ...
                    lv_obj_invalidate(tabs[currentTab].tab);
                    lvgl_port_unlock();
            ...
            //Handles CAN message receiving and screen updating
            if((esp_timer_get_time() - lastUpdateTime)/1000 > 90){
                ...
                messageNumber = 0;
                updateScreen(tabs, messageArray, messageCount);
                lastUpdateTime = esp_timer_get_time();
                usleep(0);
            }
            //Hangles re-enabling GPIO interrupt after debounce period
            if(!GPIO.pin[GPIO_INPUT_IO].int_ena && ((esp_timer_get_time() - last_interrupt_time)/1000 > 400)){
                // disabled = 0;
                gpio_intr_enable(GPIO_INPUT_IO);
                usleep(0);
            }
